...
st.title('Altave análise')
st.text('Faça upload do arquivo CSV, para uma análise detalhada dos acessos e presenças dos colaboradores.')
...
if uploaded_file is not None:
    ...
    if selected_user == 'Todos':
        ...
        sort_order = alt.SortField(field='Dias', order='ascending' if st.session_state.sort_ascending else 'descending')
        num_users = len(filtered_data)
        height_graph = max(300, num_users * 30)
        chart = alt.Chart(filtered_data).mark_bar().encode(
            y=alt.Y('Nome', sort=sort_order),
            x=alt.X('Dias', scale=alt.Scale(round=True, domain=[0, filtered_data['Dias'].max()], nice=False), axis=alt.Axis(tickCount=filtered_data['Dias'].max())),  # Eixo X de 1 em 1
            tooltip=['Nome', 'Dias']
        ).properties(
            height=height_graph,
            width='container'
        ...
        data_user['Dia'] = data_user['Data'].dt.strftime('%d/%m/%Y')
        
        unique_days = data_user['Dia'].unique().tolist()
        ...
        access_day = data_user[data_user['Dia'] == selected_day]
